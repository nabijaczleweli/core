...
fn build(input: OrchestratorInput) -> String {
    ...
    let command_monerod = command![
        ...
        flag!(input.want_arti; "--tx-proxy=tor,arti:{}", input.ports.arti_socks),
    ...
    let command_bitcoind = command![
        ...
        flag!(input.want_arti; "-proxy=arti:{}", input.ports.arti_socks),
        flag!("-dbcache=16384"),
    ...
    let arti_segment = if input.want_arti {
        format!(
            "\
  arti:
    container_name: arti
    ...
    expose:
      - {port_arti_socks}\
",
            port_arti_socks = input.ports.arti_socks,
            image_arti = input.images.arti.to_image_attribute(),
        )
    } else {
        String::new()
    };
...
#
# The used images for `bitcoind`, `monerod`, `electrs`, `arti` are pinned to specific hashes which prevents them from being altered by the Docker registry.
#
